...
<body>
	<fieldset>
		...
		<ul style="list-style: upper-roman;">
			<li>value속성에 직접 값 설정:<c:set var="directvar" value="100" />
			</li>
			<!-- 위 JSTL코드는 아래의 자바코드와 같다 -->
			<%-- pageContext.setAttribute("directvar", "100"); --%>
			<li>value속성에 EL로 값 설정:<c:set var="elvar" value="${directvar}" /></li>
			<li>value속성에 표현식으로 값 설정:<c:set var="expvar"
					value="<%=new java.sql.Date(new Date().getTime())%>" /></li>
			<li>시작태그와 종료 태그 사이에 값 설정:<c:set var="betweenvar">
					<h3>시작태그와 종료태그</h3>사이에 값 설정</c:set></li>
		</ul>
	...
	<fieldset>
		...
		<h2>기본 생성자로 생성</h2>
		<c:set scope="request" var="defaultmember"
			value="<%=new MemberDTO()%>" />
		<h2>스크립팅 요소(자바코드)로 출력</h2>
		<ul style="list-style: upper-roman;">
		...
		<h2>인자 생성자로 생성</h2>
		<c:set scope="request" var="argsmember"
			value='<%=new MemberDTO("KIM", "1234", "김길동", null, null)%>' />
	 ...
	 -->
	  ...
	  -->
		...
		<h4>target속성과 property 속성으로 값 설정후 EL로 출력</h4>
		<ul style="list-style: upper-roman;">
			<li>아이디:${defaultmember.id}</li>
			<li>비번:${defaultmember.pwd}</li>
			<li>이름:${defaultmember.name }</li>
		...
		<c:set target="${list[0]}" property="pwd" value="7777"  />
